...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	...
	uint8_t uart_msg_tx[100];
	uint8_t uart_msg_rx[100];
  ...
  // Initialize MCP and move to loopback mode
  MCP2515_Init();
  ...
  {
      USART2_SendString("MCP2515 Loopback mode OK\r\n");
  }
  else
  {
      USART2_SendString("MCP2515 Loopback mode failed\r\n");
  }
  ...
  while (1)
  {
	  ...
	  MCP2515_SendMessage(id,msg,dlc_len);
	  sprintf(uart_msg_tx,"Message Transmitted : ID = 0x%02X, DLC = 0x%02X, DATA1 = 0x%02X, DATA2 = 0x%02X\r\n",id,dlc_len,msg[0],msg[1]);
	  USART2_SendString(uart_msg_tx);

	  //Receiving frame
	  MCP2515_ReadMessage(&rx_id,rx_msg,&rx_dlc_len);
	  sprintf(uart_msg_rx,"Message Received : ID = 0x%02X, DLC = 0x%02X, DATA1 = 0x%02X, DATA2 = 0x%02X\r\n",rx_id,rx_dlc_len,rx_msg[0],rx_msg[1]);
