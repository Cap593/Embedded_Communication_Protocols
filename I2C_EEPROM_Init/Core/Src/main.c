...
int main(void)
{
  ...
  AT24C256_I2C1_GPIO_Init();
  AT24C256_I2C1_Init();

  USART2_SendString("I2C1 Initialized for AT24C256\r\n");

  if(AT24C256_WriteByte(0x0010, write_data) == E_OK)
  {
      USART2_SendString("EEPROM Write OK\r\n");
  }
  else
  {
      USART2_SendString("EEPROM Write Failed\r\n");
  }

  if(AT24C256_ReadByte(0x0010, &read_data) == E_OK)
  {
      sprintf(uart_msg,
              "EEPROM Read OK: Addr=0x%04X Data=0x%02X\r\n",
              0x0010,
              read_data);
  ...
  {
      USART2_SendString("EEPROM Read Failed\r\n");
  }
