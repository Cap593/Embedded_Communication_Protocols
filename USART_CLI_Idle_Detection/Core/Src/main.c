...
uint8_t msg;
volatile uint8_t idle_detected =0;
...
void ProcessReceivedFrame(void)
{
    ...
    while(rx_tail != dma_head)
    {
        cmd_buffer[index++] =
                dma_rx_buffer[rx_tail];

		msg = dma_rx_buffer[rx_tail];
		USART2_SendByte(msg);
...
int main(void)
{
  ...
  /* 500msec delay to block CPU before next DMA service */
  HAL_Delay(500);
  ...
  /* Start receiving */
  USART2_DMA_CircularReceive(dma_rx_buffer,RX_BUFFER_SIZE);
  ...
  while (1)
  {
	  if(idle_detected)
	  {
		  ...
		  /* updating/calculating the head */
		  dma_head = (RX_BUFFER_SIZE - DMA1_Stream5->NDTR) % RX_BUFFER_SIZE;
