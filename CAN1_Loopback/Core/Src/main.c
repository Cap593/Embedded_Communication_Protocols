...
int main(void)
{
  ...
  if(CAN1_Init())
  {
      USART2_SendString("CAN1 loopback init OK\r\n");
  }
  else
  {
      USART2_SendString("CAN1 loopback init failed\r\n");
  }
  ...
  while (1)
  {
	  ...
	  if(CAN1_ReadMessage_FIFO0(&node3_id, node3_data, &node3_dlc) == E_OK)
	  {
	      char node3_msg[100];

	      sprintf(node3_msg,
	              "Node3 : ID=0x%03X DLC=%d DATA0=0x%02X DATA1=0x%02X\r\n",
				  node3_id, node3_dlc, node3_data[0], node3_data[1]);

	      USART2_SendString(node3_msg);
	  }
	  else
	  {
	      USART2_SendString("CAN1 RX no message\r\n");
	  }

	  /* MCP transmission and reception
	  ...
	  //This time Node 2 Transmits and Node 1 Receives
	  MCP2515_SelectNode(MCP2515_NODE_2);
	  MCP2515_SelectSpiBus(MCP2515_SPI_BUS_1);

	  if(MCP2515_SendMessage(0x201,msg,dlc_len))
	  {
		  HAL_Delay(10);
