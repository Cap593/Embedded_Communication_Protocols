...
int main(void)
{
  ...
  // Initialize MCP node1 and move to loopback mode
  MCP2515_SelectNode(MCP2515_NODE_1);
  MCP2515_SelectSpiBus(MCP2515_SPI_BUS_2);
  MCP2515_Init();
  ...
  while((MCP2515_ReadRegister(MCP2515_CANSTAT) & MCP2515_MODE_MASK) != MCP2515_MODE_LOOPBACK);

  uint8_t canstat = MCP2515_ReadRegister(MCP2515_CANSTAT);

  if((canstat & MCP2515_MODE_MASK) == MCP2515_MODE_LOOPBACK)
  {
      USART2_SendString("MCP2515 Node1 Loopback mode OK\r\n");
  }
  else
  {
      USART2_SendString("MCP2515 Node1 Loopback mode failed\r\n");
  }

  // Initialize MCP node2 and move to loopback mode
  MCP2515_SelectNode(MCP2515_NODE_2);
  MCP2515_SelectSpiBus(MCP2515_SPI_BUS_1);
  MCP2515_Init();
  ...
  if((canstat2 & MCP2515_MODE_MASK) == MCP2515_MODE_LOOPBACK)
  {
      USART2_SendString("MCP2515 Node2 Loopback mode OK\r\n");
  }
  else
  {
      USART2_SendString("MCP2515 Node2 Loopback mode failed\r\n");
  }
  ...
  while (1)
  {
	  ...
	  //Receiving frame
	  if(MCP2515_ReadMessage(&rx_id,rx_msg,&rx_dlc_len))
	  {
	  ...
	  {
		  USART2_SendString("Node1 : No Message Received\r\n");
	  }

	  //Transmitting standard CAN frame
	  MCP2515_SelectNode(MCP2515_NODE_2);
	  MCP2515_SelectSpiBus(MCP2515_SPI_BUS_1);
	  if(MCP2515_SendMessage(0x007,msg,dlc_len))
	  {
		  sprintf(uart_msg_tx,"Node2 Message Transmitted : ID = 0x%03X, DLC = 0x%02X, DATA1 = 0x%02X, DATA2 = 0x%02X\r\n",0x007,dlc_len,msg[0],msg[1]);
		  USART2_SendString(uart_msg_tx);
	  }
	  else
	  {
	  ...
	  if(MCP2515_ReadMessage(&rx_id,rx_msg,&rx_dlc_len))
	  {
		  sprintf(uart_msg_rx,"Node2 Message Received    : ID = 0x%03X, DLC = 0x%02X, DATA1 = 0x%02X, DATA2 = 0x%02X\r\n",rx_id,rx_dlc_len,rx_msg[0],rx_msg[1]);
		  USART2_SendString(uart_msg_rx);
	  }
	  else
	  {
		  USART2_SendString("Node2 : No Message Received\r\n");
	  }
