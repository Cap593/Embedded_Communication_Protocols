...
uint8_t MCP2515_PrintTxB0Status(void)
{
	uint8_t TxErrorFlag = 1;
    uint8_t txctrl = MCP2515_ReadRegister(MCP2515_TXB0CTRL);


    if(txctrl & MCP2515_TXB0CTRL_TXREQ)
    {
        USART2_SendString("TXB0 still pending\r\n");
        TxErrorFlag = 0;
    }

    if(txctrl & MCP2515_TXB0CTRL_TXERR)
    {
        USART2_SendString("TXB0 transmit error\r\n");
        TxErrorFlag = 0;
    }

    if(txctrl & MCP2515_TXB0CTRL_MLOA)
    {
        USART2_SendString("TXB0 lost arbitration\r\n");
        TxErrorFlag = 0;
    }

    if(txctrl & MCP2515_TXB0CTRL_ABTF)
    {
        USART2_SendString("TXB0 aborted\r\n");
        TxErrorFlag = 0;
    }
